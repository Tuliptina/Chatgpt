...
v6.0 changes:
- MetadataToken now carries session_id for isolation and cleanup
- add_to_loop() accepts session_id parameter
- NEW: remove_session_tokens() — clean all tokens for a deleted session
- NEW: "tone" loop category added alongside character/plot/setting/theme/style/fact
- build_context() accepts active_sessions filter for session isolation toggle
- load_from_mnemo() extracts session_id from Mnemo metadata
- MetadataExtractor.extract_smart() routes to K2.5 (MEMORY_MODEL_ID)

v5.1: build_context() cache-only, prefix matching in update_relevance().
"""
...
class MetadataToken:
    ...
    session_id: str = ""  # v6.0: Track which session created this token
...
class MetadataExtractor:
    ...
    def extract_smart(self, content: str, category: str = "general") -> MetadataToken:
        """Smart extraction using K2.5 with Native JSON Mode."""
...
class LoopManager:
    ...
    v6.0: Added session_id tracking, tone loop, session isolation,
    remove_session_tokens(), and K2.5 model routing.
    ...
    def __init__(self, mnemo_client: MnemoClient, openrouter_key: str = None):
        ...
        # v6.0: Added "tone" loop category
        for category in ["character", "plot", "setting", "theme", "style", "tone", "fact", "general"]:
    ...
    def add_to_loop(self, content: str, category: str = "general",
                    memory_id: str = None, use_smart: bool = False,
                    session_id: str = "") -> MetadataToken:
        """Add new content to appropriate loop.
        v6.0: Accepts session_id for tracking ownership.
        """
        if use_smart:
            token = self.extractor.extract_smart(content, category)
        else:
    ...
    def remove_session_tokens(self, session_id: str) -> int:
        """v6.0: Remove all tokens belonging to a specific session.
        ...
        for loop in self.loops.values():
            before = len(loop.tokens)
            loop.tokens = [t for t in loop.tokens if t.session_id != session_id]
            removed += before - len(loop.tokens)
        # Also clean content cache entries that only this session referenced
        # (conservative: only remove if we know the ref came from this session)
        return removed

    def set_active_sessions(self, session_ids: Optional[List[str]] = None):
        """v6.0: Store active session filter for build_context().
        When set, build_context() only includes tokens from these sessions.
        Pass None to disable filtering (all sessions visible).
        """
    ...
    def build_context(self, query: str, active_sessions: Optional[List[str]] = None) -> Tuple[str, Dict]:
        """Build context string from cached loop data.

        v6.0: Accepts active_sessions filter for session isolation toggle.
        When set, only tokens from those sessions are included.
        ...
        # Use stored filter if not explicitly passed
        ...
        for loop in self.loops.values():
            for token in loop.tokens:
                # v6.0: Session isolation filter
                if active_sessions and token.session_id and token.session_id not in active_sessions:
                    continue
